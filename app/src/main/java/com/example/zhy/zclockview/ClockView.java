...
public class ClockView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private static class DrawThread extends Thread {
		private WeakReference<ClockView> mReference;

		public DrawThread(ClockView clockView) {
			mReference = new WeakReference<ClockView>(clockView);
		}

		@Override
		public void run() {
			super.run();
			ClockView clockView = mReference.get();
			if (clockView == null)
				return;
			while (clockView.mIsRuning) {
				clockView.initTime(System.currentTimeMillis());
				clockView.refreshUI();
				try {
					Thread.sleep(25);
				} catch (InterruptedException e) {
					e.printStackTrace();
					clockView.mIsRuning = false;
				}
			}
		}
